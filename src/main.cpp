...
uint256 hashGenesisBlock("0x00000a83a4b3d0f299fad8c351b264a8a3e1e4c284b58dcb722e4d27380b0161");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HCCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HCCoin Signed Message:\n";
...
{
    // HCCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
		return 100000000000 * COIN;

    nSubsidy >>= (nHeight / 5020000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 1 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 1 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 1 * 2.8;
...
{
    // HCCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "HCCoin Genesis Block!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfc, 0xcc, 0xba, 0xda }; // HCCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HCCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HCCoinMiner : ProcessBlock, block not accepted");
    }
...
void static HashcoinMiner(CWallet *pwallet)
{
    printf("HashcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Hashcoin-miner");
    ...
    {
        printf("HashcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateHashcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HashcoinMiner, pwallet));
}
